...
function getUserBySid(sid) {
  return db.dbc().collection("users").findOne({ "sid": sid })
    .then(user => {
      return user
    })
}
...
function updateOrCreateUser(profile) {
  return db.dbc().collection("users").findOne({ email: profile.email })
    .then(user => {
      if (!user) {
        ...
        return db.dbc().collection("users")
...
function updateUserSid(user) {
  return generateUniqueSid()
    .then(sid => {
      return db.dbc().collection("users").findOneAndUpdate({ _id: user._id }, { "$set": { "sid": sid } }, { "returnOriginal": false })
        .then(result => {
          return result.value
        })
    })
