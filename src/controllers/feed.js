...
function getVideosInPlaylist(googleAuth, playlistId, fromDate, maxResults, list, next) {
  ...
  .then(response => {
    unfilteredItems = response.data.items
    filteredItems = unfilteredItems.filter(item => { return new Date(item.snippet.publishedAt) > fromDate })

    newList = list.concat(filteredItems)
...
function getFeeds(subscription, user) {
  ...
  var updateFeeds = currentDate - subscription.feeds_update > UPDATE_DIFF_MILISECONDS
