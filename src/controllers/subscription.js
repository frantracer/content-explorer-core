...
function getSubscriptions (user) {
    const UPDATE_DIFF_MILLISECONDS = 60 * 60 * 1000 // 1 HOUR
    const curDate = new Date()
    const updateUserSubs = curDate - user.subs_update > UPDATE_DIFF_MILLISECONDS
    ...
    // TODO: First perform a query to check whether the channel exists in the DB
        const googleAuth = authC.createOauth2Client(user.google_profile.access_token)
        const channelIds = newSubs.map(sub => { return sub.snippet.resourceId.channelId })
